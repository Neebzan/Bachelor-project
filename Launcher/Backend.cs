...
namespace Launcher {
    ...
    //        byte [ ] msg = MessageFormatter.MessageBytes<UserModel>(user);
    ...
    //        using (TcpClient client = ConnectoToMiddleware(ConnectionTimeoutMS)) {


    //            if (client == null)
    //                return false;

    //            UserModel user = new UserModel() { UserID = username, PswdHash = hashedPassword, RequestType = RequestType.Read };
    ...
    //                string result = MessageFormatter.ReadStreamOnce(client.GetStream());
    ...
    //                        if (resultUser.RequestType == RequestType.TokenGet) {
    ...
    //        UserModel user = new UserModel() { UserID = userID, Token = token, RequestType = RequestType.TokenValidate};
    ...
    //        UserModel user = new UserModel() { UserID = username, PswdHash = hashedPassword, RequestType = RequestType.Create };
    ...
    //                UserModel resultUser = Newtonsoft.Json.JsonConvert.DeserializeObject<UserModel>(result);
    //                if (resultUser.RequestType != RequestType.Error) {
    //                    if (resultUser.RequestType == RequestType.Create) {
    //                        LoggedUser = resultUser;
    //                        client.Dispose();
    //                        return true;
    //                    }
